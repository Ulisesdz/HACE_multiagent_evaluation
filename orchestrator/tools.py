...
def crypto_prediction_tool(coin: str):
    ...
    try:
        ...
        if not columns:
            ...
            possible_match = next(
                (t for t in all_tables if clean_coin in t or coin.upper() in t), None
            )
            if possible_match:
        ...
        if not price_col:
            if len(columns) > 1:
                ...
                return f"Error estructura tabla {clean_coin}: {columns}"
        ...
        print(
            f"   └─ Info DB: Tabla='{clean_coin}', Precio='{price_col}', Orden='{order_clause}'"
        )
        ...
        query = (
            f'SELECT "{price_col}" FROM {clean_coin} ORDER BY {order_clause} LIMIT 3'
        )
...
def weather_prediction_tool(city: str):
    ...
    try:
        ...
        temp_col = None
        if "AvgTemperature" in columns:
            temp_col = "AvgTemperature"  # Kaggle standard
        elif "avg_temp" in columns:
            temp_col = "avg_temp"
        else:
            for col in columns:
                if "temp" in col.lower():
                    temp_col = col
                    break
        if not temp_col and len(columns) > 1:
            temp_col = columns[-1]  # Fallback

        if not temp_col:
            return f"Error columnas: {columns}"
        ...
        date_col = next(
            (c for c in columns if c.lower() == "date" or c.lower() == "dt"), None
        )
        ...
        print(
            f"   └─ Info DB: Tabla='{table_name}', Columna='{temp_col}', Clausula='{order_clause}'"
        )
...
def weather_rag_tool(query: str):
    ...
    vectorstore = Chroma(
        persist_directory=WEATHER_VECTOR_DB, embedding_function=embeddings
    )
    retriever = vectorstore.as_retriever(search_kwargs={"k": 2})
    docs = retriever.invoke(query)
