...
def technical_node(state):
    ...
    if not clean_instruction:
        ...
        user_msg = HumanMessage(
            content=clean_instruction, name="supervisor_instruction"
        )
    ...
    asset_name = (
        extract_asset_from_task(clean_instruction)
        if clean_instruction
        else "Activo no especificado"
    )
...
def planner_node(state):
    ...
    if not user_msg:
        return {"pending_tasks": [], "completed_outputs": []}

    # Generar plan de tareas usando salida estructurada
    plan = planner_llm.invoke([SystemMessage(content=PLANNER_SYSTEM_PROMPT), user_msg])
