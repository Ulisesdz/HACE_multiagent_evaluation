...
mlflow.langchain.autolog(disable=False)  # Disable para no crear traza aparte para llm-j


# --- CONFIGURACIÓN DE PÁGINA ---
st.set_page_config(page_title="Sistema Multi-Agente", page_icon="🤖", layout="wide")
...
with st.sidebar:
    ...
    audit_mode = st.toggle("Activar LLM-J", value=True)
    if audit_mode:
        st.info(
            "El sistema evaluará la veracidad de cada respuesta usando un LLM-as-a-judge."
        )
...
st.markdown(
    "Sistema jerárquico de agentes para predicción de Criptomonedas y Clima utilizando Modelos Locales (Llama 3.1) y RAG"
)
st.markdown(
    "Pregunta predicciones futuras o datos históricos sobre *Bitcoin, Solana o Ethereum* para criptomonedas, o clima en *Tokio, New York, Paris o Madrid* para información meteorológica."
)
...
if user_input:
    ...
    with st.chat_message("assistant", avatar="🤖"):
        ...
        run_name = f"Interaction_{len(st.session_state.messages)//2 + 1}"
        ...
        with mlflow.start_run(run_name=run_name) as run:
            try:
                ...
                # 1. EJECUCIÓN DEL AGENTE (Span Hijo 1)
                    ...
                    for node_name, node_output in event.items():
                        ...
                        if node_name == "Supervisor":
                            ...
                            status_container.write(
                                f"📡 **Supervisor:** Redirigiendo a `{next_agent}`"
                            )

                        # LOGICA DE AGENTES (Crypto y Weather)
                            ...
                            for msg in node_output["messages"]:
                                ...
                                if (
                                    ...
                                    for tool_call in msg.tool_calls:
                                        ...
                                        status_container.write(
                                            f"**{node_name}** decide usar: `{tool_name}`"
                                        )
                                        with status_container.expander(
                                            f"Inputs para {tool_name}"
                                        ):
                                            st.json(tool_args)
                                ...
                                if msg.type == "tool":
                                    ...
                                    with status_container.expander(
                                        f"Ver Output de {tool_name}"
                                    ):
                                        st.code(
                                            tool_output
                                        )  # Uso de code para que se vea bien si es CSV o tabla

                                    # Output guardado para el juez
                                    current_turn_tools.append(msg.content)
                ...
                status_container.update(
                    label="✅ Respuesta Generada", state="complete", expanded=False
                )
                ...
                # 2. EJECUCIÓN DEL JUEZ (Span Hijo 2)
                if audit_mode:
                    # Contexto para el juez
                    if not current_turn_tools:
                        context_text = "NO_DATA_RETRIEVED (El agente respondió sin usar herramientas o fallaron)."
                    else:
                        context_text = "\n---\n".join(
                            [str(c) for c in current_turn_tools]
                        )

                    # Spinner específico para el juez
                    with st.spinner("LLM-J auditando la respuesta..."):
                        mlflow.log_param("judge_context_length", len(context_text))
                    ...
                    st.divider()
                    st.markdown(
                        f"### {icon} Auditoría de Fidelidad: :{color}[{verdict.score}/10]"
                    )

                    st.info(f"**Razonamiento:** {verdict.step_by_step_analysis}")
                    st.info(f"**Tipo de Error:** {verdict.error_type}")
