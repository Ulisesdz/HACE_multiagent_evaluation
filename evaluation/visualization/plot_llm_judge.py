...
def plot_llm_judge_modules():
    ...
    if max_scale == 4:
        ...
        modules = ['llm_judge_planner', 'llm_judge_supervisor', 'llm_judge_agents', 'llm_judge_final']
    ...
    bp = ax.boxplot(data, tick_labels=available_names, patch_artist=True, showmeans=True)
    
    colors = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#FFA07A']
    for patch, color in zip(bp['boxes'], colors[:len(available_modules)]):
        patch.set_facecolor(color)
    
    ax.set_title(f'Distribución de Scores LLM-Judge por Módulo (Escala {max_scale})', 
...
def plot_error_categories():
    ...
    error_col = 'llm_judge_error_category' if 'llm_judge_error_category' in df.columns else 'error_category'
...
def plot_llm_judge_by_difficulty():
    ...
    overall_col = 'llm_judge_overall' if 'llm_judge_overall' in df.columns else 'overall_score'
    ...
    ax.set_title(f'Overall Score LLM-Judge por Dificultad (Escala {max_scale})', 
                 fontsize=14, fontweight='bold')
    ax.set_ylabel(ylabel)
    ax.set_ylim(ylim)
    ax.grid(True, axis='y', alpha=0.3)
