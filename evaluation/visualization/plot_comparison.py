...
def load_comparison_data_all():
    ...
    if accumulated_path.exists():
        ...
        numeric_cols = [
            ...
            'llm_judge_overall', 'llm_judge_time',
            'mace_score', 'mace_time', 'mace_layer3_used'
        ]
        ...
        if has_baseline.any():
            df_merged.loc[has_baseline, 'baseline_score'] = df_raw.loc[has_baseline, 'baseline_score']
            df_merged.loc[has_baseline, 'baseline_eval_time'] = df_raw.loc[has_baseline, 'baseline_time']
        ...
        if has_llm.any():
            df_merged.loc[has_llm, 'llm_judge_overall'] = df_raw.loc[has_llm, 'llm_judge_overall']
            df_merged.loc[has_llm, 'llm_judge_eval_time'] = df_raw.loc[has_llm, 'llm_judge_time']
            # Normalizar 1-4 → 0-1
        ...
        if has_mace.any():
            ...
            df_merged.loc[has_mace, 'mace_eval_time'] = df_raw.loc[has_mace, 'mace_time']
            df_merged.loc[has_mace, 'mace_layer3_used'] = df_raw.loc[has_mace, 'mace_layer3_used']
        ...
        if baseline_path.exists():
            ...
            df_baseline = df_baseline.rename(columns={
                'id': 'query_id',
                'evaluation_time': 'baseline_eval_time'
            })
        ...
        if mace_path.exists():
            ...
            df_mace = df_mace.rename(columns={
                'id': 'query_id',
        ...
        if not df_baseline.empty:
            df_merged = df_baseline[['query_id', 'baseline_score', 'baseline_eval_time', 'difficulty']].copy()
...
def plot_score_comparison_triple():
    ...
    if 'baseline_score' in df.columns and df['baseline_score'].notna().any():
        axes[0].hist(df['baseline_score'].dropna(), bins=20, alpha=0.5, 
                    label=f'Baseline (n={df["baseline_score"].notna().sum()})', 
                    color='steelblue', edgecolor='black')
        plotted_any = True
    
    if 'llm_judge_normalized' in df.columns and df['llm_judge_normalized'].notna().any():
        axes[0].hist(df['llm_judge_normalized'].dropna(), bins=20, alpha=0.5, 
                    label=f'LLM-Judge (n={df["llm_judge_normalized"].notna().sum()})', 
                    color='coral', edgecolor='black')
        plotted_any = True
    
    if 'mace_score' in df.columns and df['mace_score'].notna().any():
        axes[0].hist(df['mace_score'].dropna(), bins=20, alpha=0.5, 
                    label=f'MACE (n={df["mace_score"].notna().sum()})', 
                    color='mediumpurple', edgecolor='black')
        plotted_any = True
    
    if plotted_any:
        axes[0].set_title('Distribución de Scores Globales (Normalizados a 0-1)', fontweight='bold')
        axes[0].set_xlabel('Score (0-1)')
        axes[0].set_ylabel('Frecuencia')
        axes[0].legend()
        axes[0].grid(True, alpha=0.3)
    else:
        axes[0].text(0.5, 0.5, 'No hay datos de scores disponibles', 
    ...
    if len(data_to_plot) >= 2:
        bp = axes[1].boxplot(data_to_plot, tick_labels=labels_plot, 
        ...
        axes[1].text(0.5, 0.5, 'Se necesitan al menos 2 métodos para comparar', 
                     ha='center', va='center', transform=axes[1].transAxes)
        axes[1].axis('off')
    
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}comparison_scores_triple.png", dpi=300, bbox_inches='tight')
...
def plot_time_comparison_triple():
    ...
    ax.set_title('Comparación de Tiempos de Evaluación (3 Métodos)', 
    ...
    if times['MACE'] and times['LLM-Judge']:
        ...
        ax.text(0.5, 0.95, f'MACE es {speedup:.0f}% más rápido que LLM-Judge', 
...
def plot_difficulty_comparison_triple():
    ...
    if 'baseline_score' in df.columns and df['baseline_score'].notna().any():
        grouped_data['Baseline'] = df.groupby('difficulty')['baseline_score'].mean().reindex(difficulty_order)
        print(f"   Baseline por difficulty: {grouped_data['Baseline'].notna().sum()} niveles")
    
    if 'llm_judge_normalized' in df.columns and df['llm_judge_normalized'].notna().any():
        grouped_data['LLM-Judge'] = df.groupby('difficulty')['llm_judge_normalized'].mean().reindex(difficulty_order)
        print(f"   LLM-Judge por difficulty: {grouped_data['LLM-Judge'].notna().sum()} niveles")
    
    if 'mace_score' in df.columns and df['mace_score'].notna().any():
        grouped_data['MACE'] = df.groupby('difficulty')['mace_score'].mean().reindex(difficulty_order)
    ...
    for i, (method, data) in enumerate(grouped_data.items()):
        ...
        bars = ax.bar(x + offset, data, width, label=method, 
                      color=colors.get(method, 'gray'), edgecolor='black', alpha=0.7)
    ...
    ax.set_title('Desempeño por Dificultad: Comparación de 3 Métodos (Normalizados a 0-1)', 
                 fontsize=14, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(f"{OUTPUT_DIR}comparison_by_difficulty_triple.png", dpi=300, bbox_inches='tight')
...
def plot_summary_table_triple():
    ...
    if 'baseline_score' in df.columns and df['baseline_score'].notna().any():
        ...
        if 'baseline_eval_time' in df.columns and df['baseline_eval_time'].notna().any():
            summary_data['Tiempo Promedio'].append(f"{df['baseline_eval_time'].mean():.3f}s")
        else:
    ...
    if 'llm_judge_normalized' in df.columns and df['llm_judge_normalized'].notna().any():
        ...
        if 'llm_judge_eval_time' in df.columns and df['llm_judge_eval_time'].notna().any():
            summary_data['Tiempo Promedio'].append(f"{df['llm_judge_eval_time'].mean():.2f}s")
        else:
    ...
    if 'mace_score' in df.columns and df['mace_score'].notna().any():
        ...
        if 'mace_eval_time' in df.columns and df['mace_eval_time'].notna().any():
            summary_data['Tiempo Promedio'].append(f"{df['mace_eval_time'].mean():.3f}s")
        else:
        ...
        if 'mace_layer3_used' in df.columns:
            ...
            if layer3_count > 0:
                ...
                summary_data['Características'].append(f'Híbrido, Layer 3: {layer3_pct:.0f}%')
            else:
    ...
    plt.title(f'Resumen Comparativo: Baseline vs LLM-Judge vs MACE', 
              fontsize=14, fontweight='bold', pad=20)
    plt.savefig(f"{OUTPUT_DIR}comparison_summary_table_triple.png", dpi=300, bbox_inches='tight')
