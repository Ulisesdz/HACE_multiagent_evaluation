...
def plot_baseline_metrics_distribution():
    ...
    for col, title, ax in metrics:
        if col in df.columns:
            ...
            ax.text(0.5, 0.5, f'Columna {col} no disponible', 
...
def plot_hallucination_analysis():
    ...
    halluc_col = 'baseline_hallucination_rate' if 'baseline_hallucination_rate' in df.columns else 'hallucination_rate'
    ...
    axes[0].axvline(df[halluc_col].mean(), color='red', linestyle='--', linewidth=2, 
    ...
    if 'category' in df.columns:
        df_halluc = df[df[halluc_col] > 0.1].groupby('category').size().sort_values(ascending=False).head(10)
        ...
        if len(df_halluc) > 0:
            ...
            axes[1].text(0.5, 0.5, 'No hay casos con >10% alucinaciones', 
                        ha='center', va='center', fontsize=12)
            axes[1].axis('off')
    else:
        axes[1].text(0.5, 0.5, 'Columna "category" no disponible', 
                    ha='center', va='center', fontsize=12)
        axes[1].axis('off')
...
def plot_sql_correctness_breakdown():
    ...
    sql_correct_col = 'baseline_sql_correctness' if 'baseline_sql_correctness' in df.columns else 'sql_correctness'
    ...
    if sql_correct_col in df_sql.columns:
        axes[0].hist(df_sql[sql_correct_col], bins=10, edgecolor='black', alpha=0.7, color='lightblue')
        axes[0].axvline(df_sql[sql_correct_col].mean(), color='red', linestyle='--', linewidth=2, 
        ...
        axes[0].text(0.5, 0.5, f'Columna {sql_correct_col} no disponible', 
                    ha='center', va='center')
        axes[0].axis('off')
    ...
    if 'sql_violations' in df_sql.columns:
        ...
        axes[1].text(0.5, 0.5, 'Datos de violaciones no disponibles\n(solo en CSV legacy)', 
