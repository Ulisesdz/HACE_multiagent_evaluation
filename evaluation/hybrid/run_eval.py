...
def run_hybrid_evaluation():
    ...
    for idx, case in enumerate(dataset, 1):
        ...
        try:
            ...
            result = {
                ...
                'layer3_score': float(evaluation['layer3_score']) if evaluation['layer3_score'] is not None else None,
    ...
    summary_cols = [
        ...
        'hybrid_score', 'quality_label', 'confidence',
        'layer1_score', 'layer2_score', 'layer3_score',
        'layer3_used', 'evaluation_time'
    ]
    ...
    if df['layer3_score'].notna().any():
        print(f"  • Layer 3 (LLM):        {df['layer3_score'].mean():.3f} (cuando se usa)")
    
    print(f"\nUso de Layer 3:")
    layer3_count = df['layer3_used'].sum()
    print(f"  • Casos con Layer 3:  {layer3_count} ({layer3_count/len(df)*100:.1f}%)")
    print(f"  • Casos sin Layer 3:  {len(df)-layer3_count} ({(len(df)-layer3_count)/len(df)*100:.1f}%)")
    ...
    print(f"\nPor Categoría (Top 5):")
    cat_stats = df.groupby('category')['hybrid_score'].agg(['mean', 'count']).sort_values('mean', ascending=False).head(5)
    print(cat_stats)
    ...
    if layer3_count > 0:
        ...
        print(f"  • Layer 3: {layer3_cases['layer3_time'].mean():.3f}s (avg cuando se usa)")
