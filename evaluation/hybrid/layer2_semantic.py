...
class SemanticEvaluator:
    ...
    def __init__(self):
        ...
        try:
            ...
            print("   [WARNING] sentence-transformers no instalado. Capa 2 usará solo heurísticas.")
    ...
    def evaluate_all(self, trace_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        results['task_fidelity'] = self.evaluate_task_fidelity(
            trace_data.get('user_question', ''),
            trace_data.get('planner_tasks', [])
        )
        ...
        for execution in trace_data.get('agent_executions', []):
            fidelity = self.evaluate_agent_fidelity(
                execution.get('tool_outputs', ''),
                execution.get('agent_response', '')
            )
        ...
        results['evaluation_method'] = 'embeddings' if self.available else 'heuristics_only'
    ...
    def _task_fidelity_heuristic(self, user_query: str, planner_tasks: List[str]) -> Dict:
        """
    ...
    def evaluate_routing_keywords(self, routing_trace: List[Dict]) -> Dict:
        ...
        keyword_map = {
            'Fundamental_Analyst': {
                'high': ['qué es', 'explicar', 'concepto', 'definición', 'noticias', 'contexto'],
        ...
        avg_confidence = np.mean([r['confidence'] for r in routing_details]) if routing_details else 0.5
    ...
    def evaluate_report_completeness(self, planner_tasks: List[str], final_report: str) -> Dict:
        """
        ...
        coverage = 1.0 - (len(missing_assets) / len(mentioned_assets)) if mentioned_assets else 1.0
    ...
    def _number_in_set(self, number: float, number_set: set, tolerance: float = 0.01) -> bool:
        """
