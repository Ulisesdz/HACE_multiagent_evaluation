...
class BaselineMetricsCalculator:
    ...
    def evaluate_routing_accuracy(self, routing_trace: List[Dict]) -> Dict[str, any]:
        """
        ...
        avg_precision = (
            sum(m["precision"] for m in metrics_per_class.values()) / num_agents
            if num_agents > 0
            else 0
        )
        avg_recall = (
            sum(m["recall"] for m in metrics_per_class.values()) / num_agents
            if num_agents > 0
            else 0
        )
        avg_f1 = (
            sum(m["f1"] for m in metrics_per_class.values()) / num_agents
            if num_agents > 0
            else 0
        )
    ...
    def _infer_expected_agent(self, task: str) -> str:
        ...
        risk_keywords = ["volatilidad", "riesgo", "seguro", "peligro", "calcular"]
    ...
    def evaluate_numeric_accuracy(
        self, tool_outputs: List[str], agent_responses: List[str]
    ) -> Dict[str, float]:
        """
        ...
        for tool_out, agent_resp in zip(tool_outputs, agent_responses):
            ...
            tp = len(
                [n for n in agent_numbers if self._is_in_tolerance(n, tool_numbers)]
            )

            # False Positives: números del agente que NO están en tool
            fp = len(
                [n for n in agent_numbers if not self._is_in_tolerance(n, tool_numbers)]
            )

            # False Negatives: números del tool que NO reportó el agente
            fn = len(
                [n for n in tool_numbers if not self._is_in_tolerance(n, agent_numbers)]
            )
        ...
        avg_precision = (
            sum(total_precision) / len(total_precision) if total_precision else 1.0
        )
        avg_recall = sum(total_recall) / len(total_recall) if total_recall else 1.0
        avg_hallucination = (
            sum(hallucinations) / len(hallucinations) if hallucinations else 0.0
        )
    ...
    def _is_in_tolerance(
        self, number: float, number_list: List[float], tolerance=0.01
    ) -> bool:
    ...
    def evaluate_task_coverage(
        self, planned_tasks: List[str], completed_tasks: List[str]
    ) -> Dict[str, any]:
        """
        ...
        completed_tasks_clean = [
            t for t in completed_tasks if t and isinstance(t, str) and t.strip()
        ]
    ...
    def _validate_sql_patterns(self, task: str, sql: str) -> List[str]:
        ...
        if any(
            kw in task_lower
            for kw in ["top", "más alto", "más bajo", "máximo", "mínimo"]
        ):
        ...
        if any(
            kw in task_lower for kw in ["último", "últimos", "reciente", "recientes"]
        ):
            if "ORDER BY" not in sql_upper:
