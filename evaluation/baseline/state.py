...
class RoutingMetrics(BaseModel):
    ...
    per_class: Dict[str, Dict[str, float]] = Field(
        description="Métricas por agente", default_factory=dict
    )
    confusion_matrix: Dict[str, Dict[str, int]] = Field(
        description="Matriz de confusión", default_factory=dict
    )
...
class NumericMetrics(BaseModel):
    ...
    precision: float = Field(
        description="% de números del agente que están en tool (0-1)"
    )
    recall: float = Field(
        description="% de números del tool reportados por el agente (0-1)"
    )
...
class SQLMetrics(BaseModel):
    ...
    violations: List[str] = Field(
        description="Lista de patrones violados", default_factory=list
    )
    total_queries: int = Field(
        description="Número total de queries evaluadas", default=0
    )
