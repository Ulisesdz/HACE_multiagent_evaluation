...
def run_baseline_batch_evaluation():
    ...
    for idx, case in enumerate(dataset, 1):
        ...
        try:
            ...
            print(
                f"   • Hallucination:    {baseline_eval.numeric_metrics.hallucination_rate:.1%}"
            )
            print(
                f"   • Task Coverage:    {baseline_eval.task_coverage_metrics.coverage:.1%}"
            )
            ...
            result = {
                ...
                "routing_per_class": (
                    str(baseline_eval.routing_metrics.per_class)
    ...
    print(f"\nPor Categoría (Top 5):")
    cat_stats = (
        df.groupby("category")["baseline_score"]
        .agg(["mean", "count"])
