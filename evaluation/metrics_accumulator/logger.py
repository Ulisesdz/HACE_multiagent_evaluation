...
class MetricsLogger:
    ...
    def log_online_evaluation(
        ...
        if baseline_eval:
            record.update({
                ...
                'baseline_hallucination_rate': float(baseline_eval.numeric_metrics.hallucination_rate),
                'baseline_task_coverage': float(baseline_eval.task_coverage_metrics.coverage),
                'baseline_sql_correctness': float(baseline_eval.sql_metrics.correctness),
                'baseline_time': float(baseline_eval.evaluation_time)
            })
        else:
            record.update({k: None for k in [
                'baseline_score', 'baseline_routing_f1', 'baseline_numeric_f1',
                'baseline_hallucination_rate', 'baseline_task_coverage',
                'baseline_sql_correctness', 'baseline_time'
            ]})
        ...
        if llm_judge_data:
            ...
            record.update({k: None for k in [
                'llm_judge_overall', 'llm_judge_planner', 'llm_judge_supervisor',
                'llm_judge_agents', 'llm_judge_final', 'llm_judge_error_category',
                'llm_judge_time'
            ]})
        ...
        if hybrid_eval:
            record.update({
                ...
                'mace_layer3': float(hybrid_eval['layer3_score']) if hybrid_eval['layer3_score'] is not None else None,
                'mace_layer3_used': int(hybrid_eval['layer3_used']),
                'mace_time': float(hybrid_eval['evaluation_time']),
            })
        else:
            record.update({k: None for k in [
                'mace_score', 'mace_quality', 'mace_confidence',
    ...
    def get_statistics(self, source: str = 'all') -> Dict[str, Any]:
        ...
        if source == 'all':
            ...
            raise ValueError(f"Invalid source: {source}. Must be 'online', 'offline', or 'all'")
        ...
        stats = {
            ...
            "baseline_avg_score": float(df['baseline_score'].mean()) if 'baseline_score' in df and df['baseline_score'].notna().any() else None,
            "llm_judge_avg_score": float(df['llm_judge_overall'].mean()) if 'llm_judge_overall' in df and df['llm_judge_overall'].notna().any() else None,
            "hybrid_avg_score": float(df['mace_score'].mean()) if 'mace_score' in df and df['mace_score'].notna().any() else None,
            "avg_hallucination_rate": float(df['baseline_hallucination_rate'].mean()) if 'baseline_hallucination_rate' in df and df['baseline_hallucination_rate'].notna().any() else None,
            "error_categories": df['llm_judge_error_category'].value_counts().to_dict() if 'llm_judge_error_category' in df else {},
            "date_range": {
