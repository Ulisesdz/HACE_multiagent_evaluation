...
def train_all_models():
    ...
    for table in tables:
        ...
        col_date = next((c for c in df_full.columns if 'Date' in str(c) or 'date' in str(c)), None)
        ...
        plt.plot(dates_test, y_test.values, label='Precio Real', color='blue', linewidth=2, alpha=0.7)
        plt.plot(dates_test, predictions, label='Predicción', color='orange', linestyle='--', linewidth=2)
        
        plt.title(f"Validación Modelo: {table}\nMAE: ${mae:.2f} | R2: {r2:.4f} | Acc: {accuracy_perc:.1f}%")
        plt.xlabel("Fecha")
