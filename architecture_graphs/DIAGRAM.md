...
flowchart TB
    ...
    L1_Checks --> L1_Score[Score: 0.0 - 1.0]
    ...
    L2_Metrics --> L2_Score[Score: 0.0 - 1.0]
    
    L2_Score --> Decision{Escalate to<br/>Layer 3?}
    
    Decision -->|"NO<br/>Score ≥ 0.75<br/>No Critical Failures"| Skip[Skip Layer 3<br/>Time: ~0.55s]
    
    Decision -->|"YES<br/>Score < 0.75<br/>OR Critical Failures"| Layer3{Layer 3: LLM-Judge<br/>GPT-4o}
    
    Layer3 -->|Rubric-Guided| L3_Rubric[["Evaluate by Module:<br/>• Planner (1-4)<br/>• Supervisor (1-4)<br/>• Agents (1-4)<br/>• Final Output (1-4)"]]
    L3_Rubric --> L3_Score[Score: 0.25 - 1.0<br/>Time: ~214s]
    ...
    Weights -->|NO| Fast["Final Score =<br/>0.4 × L1 + 0.6 × L2"]
    Weights -->|YES| Deep["Final Score =<br/>0.3 × L1 + 0.4 × L2 + 0.3 × L3"]
