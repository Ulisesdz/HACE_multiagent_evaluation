...
def load_data():
    ...
    numeric_cols = [
        ...
        'mace_layer1', 'mace_layer2', 'mace_layer3'
    ]
...
st.markdown("**Multi-layered Agent Consensus Evaluator** - Sistema Híbrido de Evaluación")
...
with tab2:
    ...
    if mace_count == 0:
        ...
        with col1:
            ...
            fig_layers.add_trace(go.Box(
                y=df_mace['mace_layer1'],
                name='Layer 1',
                marker_color='#3498db'
            ))
            
            fig_layers.add_trace(go.Box(
                y=df_mace['mace_layer2'],
                name='Layer 2',
                marker_color='#e74c3c'
            ))
        ...
        with col2:
            ...
            fig_pie.update_layout(
                title="Distribución de Uso de Layer 3",
                height=400
            )
        ...
        if df_mace['mace_layer3'].notna().any():
            ...
            if correlation > 0.7:
                st.success(f"✅ Alta correlación ({correlation:.3f}): Los embeddings predicen bien el score LLM")
            elif correlation > 0.4:
                st.info(f"ℹ️ Correlación moderada ({correlation:.3f}): Algunas discrepancias entre Layer 2 y 3")
            else:
                st.warning(f"⚠️ Baja correlación ({correlation:.3f}): Embeddings y LLM discrepan frecuentemente")
        ...
        escalation_pivot = df_mace.groupby('layer2_bin')['mace_layer3_used'].agg(['sum', 'count'])
        escalation_pivot['escalation_rate'] = (escalation_pivot['sum'] / escalation_pivot['count']) * 100
        ...
        st.caption("💡 **Insight:** Scores bajos en Layer 2 deberían correlacionar con mayor escalación a Layer 3")
...
with tab3:
    ...
    if times:
        ...
        with col2:
            # Speedup calculation
            if len(times) >= 2:
                baseline_t = next((t['Tiempo (s)'] for t in times if t['Método'] == 'Baseline'), None)
                llm_t = next((t['Tiempo (s)'] for t in times if t['Método'] == 'LLM-Judge'), None)
                mace_t = next((t['Tiempo (s)'] for t in times if t['Método'] == 'MACE'), None)
                
                st.markdown("### Speedup Comparativo")
                
                if mace_t and llm_t:
                    speedup_llm = ((llm_t - mace_t) / llm_t) * 100
                    st.success(f"✅ MACE es **{speedup_llm:.1f}%** más rápido que LLM-Judge")
                ...
                if mace_t and baseline_t:
                    ...
                    if slowdown > 0:
                        if slowdown > 100:
                            ...
                            st.info(f"ℹ️ MACE es **×{multiplier:.0f} más lento** que Baseline (esperado: Baseline es puramente determinista)")
                        else:
                            st.info(f"ℹ️ MACE es **{slowdown:.1f}% más lento** que Baseline")
                    else:
                        st.success(f"✅ MACE es **{abs(slowdown):.1f}% más rápido** que Baseline")
        ...
        if mace_count > 0:
            ...
            if len(no_layer3) > 0 and len(with_layer3) > 0:
                ...
                fig_layer3_time.add_trace(go.Box(
                    y=no_layer3,
                    name='Sin Layer 3',
                    marker_color='#2ecc71'
                ))
...
with tab4:
    ...
    if difficulty_data:
        ...
        st.dataframe(pivot_table.style.format("{:.3f}").background_gradient(cmap='RdYlGn', axis=None))
...
with tab5:
    ...
    default_columns = [
        'query_id', 'user_query', 'difficulty', 'category',
        'baseline_score', 'llm_judge_overall', 'mace_score',
        'mace_quality', 'mace_confidence', 'mace_layer3_used'
    ]
